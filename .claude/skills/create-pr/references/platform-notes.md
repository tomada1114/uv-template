...
- The `smart-commit` skill is a shared skill reference and is available through the repository skill bridge.
...
- Dynamic context injection is replaced by explicit command execution, so the workflow spends a few extra steps collecting state but preserves the same inputs.
